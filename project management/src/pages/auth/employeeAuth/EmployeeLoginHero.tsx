...
export function EmployeeLoginHero() {
  ...
  const handleSubmit =async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/employeeLogin', {
        email,
        password,
        projectCode
      });
  ...
  return (
    <div className="form-container  flex justify-center align-center bg-[#0f4841f7]">
      ...
      <form className="my-8" onSubmit={handleSubmit}>
        ...
        <LabelInputContainer className="mb-4">
          <Label htmlFor="password">Project Code</Label>
          <Input id="projectCode" placeholder="DFDFD343" type="text"   value={projectCode}
        ...
        <div className='text-center mt-4 text-lime-100 font-normal text-sm'>
              <p>Do not have an account? <Link to="/auth/employeeSignup" className='font-medium text-white'>Signup</Link></p>
            
            </div>
            <div className='text-center mt-3 text-lime-100 font-normal text-sm'>
              <p>Do you want to manage you project? <Link to="/auth/userLogin" className='font-medium text-white'>Login</Link></p>
            </div>
      </form>
    </div>
