...
export function EmployeeSignUpHero() {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/employeeRegister', {
        name,
        email,
        password,
        mobile,
              ...
              <Form className="my-8">
                <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4">
                  <LabelInputContainer>
                    ...
                    <Field id="firstName" name="firstName" placeholder="Tyler" type="text" as={Input} />
                    <ErrorMessage name="firstName" component="div" className="text-red-600 font-semibold text-sm" />
                  </LabelInputContainer>
                  <LabelInputContainer>
                    <Label htmlFor="lastName">Last name</Label>
                    <Field id="lastName" name="lastName" placeholder="Durden" type="text" as={Input} />
                    <ErrorMessage name="lastName" component="div" className="text-red-600 font-semibold text-sm" />
                  </LabelInputContainer>
                </div>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="email">Email</Label>
                  <Field id="email" name="email" placeholder="projectmayhem@fc.com" type="email" as={Input} />
                  <ErrorMessage name="email" component="div" className="text-red-600 font-semibold text-sm" />
                </LabelInputContainer>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="password">Password</Label>
                  <Field id="password" name="password" placeholder="••••••••" type="password" as={Input} />
                  <ErrorMessage name="password" component="div" className="text-red-600 font-semibold text-sm" />
                </LabelInputContainer>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="confirmPassword">Confirm Password</Label>
                  <Field id="confirmPassword" name="confirmPassword" placeholder="••••••••" type="password" as={Input} />
                  <ErrorMessage name="confirmPassword" component="div" className="text-red-600 font-semibold text-sm" />
                </LabelInputContainer>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="mobile">Mobile Number</Label>
                  <Field id="mobile" name="mobile" placeholder="9999999999" type="text" as={Input} />
                  <ErrorMessage name="mobile" component="div" className="text-red-600 font-semibold text-sm" />
                </LabelInputContainer>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="jobRole">Job Role</Label>
                  <Field id="jobRole" name="jobRole" placeholder="Designer" type="text" as={Input} />
                  <ErrorMessage name="jobRole" component="div" className="text-red-600 font-semibold text-sm" />
                </LabelInputContainer>
                <LabelInputContainer className="mb-4">
                  <Label htmlFor="projectCode">Project Code</Label>
                  <Field id="projectCode" name="projectCode" placeholder="DFDFD" type="text" as={Input} />
...
const LabelInputContainer = ({ children, className }:{  children: React.ReactNode;
  className?: string}) => (
